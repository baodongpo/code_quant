...
**问题描述**：
个股分析页面（`/`）中，主图（K线）与第 1 个副图（MACD）之间的底部滑动条联动正常，但第 2 个及以后的副图（RSI、KDJ 等）滑块无法跟随联动，各副图的时间轴显示范围互相独立。

**预期行为**：拖动任意一个图表的滑动条，所有图表（主图 + 全部副图）的显示范围同步联动。

**修复范围**：`web/src/components/` 中各图表组件的 ECharts `dataZoom` 配置，需通过 `dataZoom` 的 `id` 关联或 ECharts group/connect 机制实现跨图联动。
