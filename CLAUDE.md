...
- [x] 迭代9（已完成 2026-04-15，tag v0.9.1）：
  - [x] FEAT-yfinance：新增 yfinance 美股数据源，替代富途获取美股K线数据
  - [x] FEAT-multi-source：多数据源路由架构（A股/港股→富途，美股→yfinance）
  - [x] FEAT-us-adjust：美股复权因子获取与前复权计算
  - [x] FEAT-us-sync：美股K线增量同步（日K/周K/月K）
- [x] 迭代10（已完成 2026-04-15，tag v0.9.2）：
  - [x] FEAT-tushare：TuShare 美股数据源替代 yfinance（因 yfinance IP 封禁严重）
